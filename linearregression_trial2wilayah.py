...
for i in range(19):
    ...
    if i==0:
        ...
        export_csv = df.to_csv(r'/home/leathea/Downloads/Machine-Learning/PolynomialRegression/'+str(i)+r'.csv', index = None, header=True) 
        ...
        baris= range(1, (54*i)+1)
        #read dataset csv
        data_year_x = pd.read_csv("maize.csv", usecols = ['Year'], skiprows = baris, nrows = 54)
        x = np.array(data_year_x)
        x_list = x.ravel()
        data_crop_y = pd.read_csv("maize.csv", usecols = ['Crop'], skiprows = baris, nrows = 54)
        y = np.array(data_crop_y)
        ...
        # baca nama wilayah dan kode
        wilayah = pd.read_csv("maize.csv", usecols = ['Entity'], skiprows = baris, nrows = 54)
        wilayah = np.array(wilayah)
        wilayah = wilayah.ravel()
        wil_tambahan = wilayah[0:6]
        wilayah=np.concatenate((wilayah, wil_tambahan))
        kode = pd.read_csv("maize.csv", usecols=['Code'], skiprows = baris, nrows = 54)
        kode = np.array(kode)
        kode = kode.ravel()
        kode_tambahan = kode[0:6]
        kode=np.concatenate((kode, kode_tambahan))
